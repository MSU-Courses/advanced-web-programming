...
Наиболее распространенная схема – пользователь вводит логин (или email) и пароль. Сервер проверяет эти данные по базе пользователей. Если они совпадают – пользователь считается подлинным, и ему открывается доступ к закрытым разделам.
...
- **bcrypt**. Медленный и устойчивый к атакам перебора. Поддерживается нативно в PHP через `password_hash()`.
...
Для надёжной работы с паролями PHP предоставляет функции
...
- `PASSWORD_DEFAULT` автоматически выбирает актуальный надёжный алгоритм (в данный момент — `bcrypt`).
- Полученный хэш сохраняется в базе данных вместо оригинального пароля.
...
**Регистрация пользователя** — это процесс создания новой учётной записи, позволяющей получить доступ к закрытым разделам сайта или приложения. Обычно включает следующие шаги:

1. Пользователь заполняет форму, указывая логин (или email) и пароль.
2. Сервер проверяет, существует ли уже пользователь с таким логином.
3. Если пользователь не найден, пароль хэшируется с помощью `password_hash()`, а данные сохраняются в базе.
...
if (empty($errors)) {
    ...
    saveUserToDatabase($username, $passwordHash, $email);
...
**Аутентификация** — это процесс подтверждения личности пользователя на основе введённых им данных. Он состоит из нескольких этапов:
...
function getCurrentUser() {
    return isAuthenticated() ? getUserById($_SESSION['user_id']) : null;
}
