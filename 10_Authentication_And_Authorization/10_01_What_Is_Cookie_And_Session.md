...
- `expire` – время жизни cookie (в секундах). Если 0 или не указано, cookie считается сессионным и будет храниться до закрытия браузера​. Если указать временную метку в будущем (_например_, текущее время + число секунд), cookie будет храниться до этой даты.
