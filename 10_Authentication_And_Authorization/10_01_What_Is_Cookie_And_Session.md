...
- `expire` – время жизни cookie (Unix timestamp в секундах). Если 0 или не указано, cookie считается сессионным и будет храниться до закрытия браузера​. Если указать временную метку в будущем (_например_, текущее время + число секунд), cookie будет храниться до этой даты.
...
- `httponly` – флаг, запрещающий доступ к cookie из JavaScript (HttpOnly cookie). Этот параметр нельзя указать напрямую позиционно, но его можно включить начиная с PHP 5.2 вторым булевым аргументом или передать в массив опций (в новых версиях PHP).
...
При использовании сессий и куки необходимо учитывать вопросы безопасности:

1. **Не храните чувствительные данные в куки**. Пароли, номера паспортов, личные сообщения и прочая конфиденциальная информация не должны сохраняться в виде куки на стороне клиента​ [^3]. Если нужно сохранить что-то важное между визитами, лучше сохранить идентификатор, по которому сервер затем загрузит данные (_например_, ID пользователя или токен доступа), а сами данные держать на сервере.
2. **Защищайте идентификатор сессии**. оскольку сессия идентифицируется по уникальному ID (например, значению куки PHPSESSID), крайне важно защитить этот идентификатор от перехвата. Используйте HTTPS-протокол, чтобы все куки передавались в зашифрованном виде. – это предотвращает их утечку через прослушивание трафика. Кроме того, включайте флаг `HttpOnly` для куки сессии – тогда JavaScript в браузере не сможет прочитать куки (что затрудняет кражу сессии через XSS-уязвимость). В PHP можно задать эти параметры через функцию `session_set_cookie_params()` или соответствующие директивы в `php.ini` (например, `session.cookie_secure = On` и `session.cookie_httponly = On`).
