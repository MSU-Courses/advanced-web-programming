...
Флаг `HttpOnly` — это параметр, который указывается при установке куки и ограничивает доступ к ней только сервером. При установленном флаге браузер не позволяет JavaScript-коду считывать содержимое такой куки.

Это особенно важно для защиты от XSS-атак: даже если злоумышленнику удастся внедрить вредоносный JS-скрипт на страницу, он не сможет получить доступ к куки с флагом `HttpOnly`.

Данный флаг следует устанавливать в случае, если в куки сохраняются важные данные, такие как идентификатор сессии или токены доступа. Его использование значительно повышает безопасность веб-приложения.

В PHP флаг `HttpOnly` можно установить при создании куки с помощью функции `setcookie()`:

```php
setcookie("param", $param, time() + 3600, "/", "", true, true);
```
